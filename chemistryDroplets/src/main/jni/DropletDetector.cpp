...
/*void DropletDetector::filterCandidates2(Mat contourOutlines, Mat grayBinary, vector<Droplet> candidates) {
	Mat hsv;
	Scalar h_min(80, 254, 254), h_max(92, 255, 255), o_min(100, 149, 200), o_max(130, 255, 255), he_min(105, 79, 200), he_max(179, 132, 255), li_min(47, 15, 140), li_max(83, 40, 160),
			n_min(140, 70, 90), n_max(160, 200, 110), c_min(148, 240, 90), c_max(152, 255, 255);
	...
	for (int i = 0; i < cSize; i++) {
		...
		__android_log_print(ANDROID_LOG_INFO, "TAG", "H: %d", 10.1);
		...
		else if(isBiggerScalar(hsvAverage, c_min) && isBiggerScalar(c_max, hsvAverage)) {
			bool exists = false;
			Droplet carbon = candidates[i];
			nitrogen.setName("C");
			nitrogen.setColor(Scalar(10, 10, 200));
			nitrogen.setENeg(2.55);
